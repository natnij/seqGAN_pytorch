...
class Rollout(nn.Module):
    ...
    def forward(self, sentence, given_num, ignored_tokens=None):
        ...
        self.lstm_out, self.hidden = self.lstm(embeds_existing, self.hidden)
        ...
        for i in range(given_num, SEQ_LENGTH):
            ...
            self.hidden = self.lstmCell(embeds_rollout, self.hidden)
            ...
            self.lstm_out = torch.cat((self.lstm_out, self.hidden[0].view(-1,1,self.hidden[0].shape[-1])),dim=1)
            ...
            self.y_pred_output = torch.cat((self.y_pred_output.int(), x_t.int()),dim=1)
    ...
    with torch.no_grad():
        ...
        for i in range(ROLLOUT_ITER):
            ...
            for given_num in range(1, SEQ_LENGTH):
                rollout.hidden = rollout.init_hidden(batch_size)
...
def sanityCheck_rollout_updateParams(batch_size=1):
    ...
    try:
        ...
        param_r = [str(x) for x in list(rollout.lstm.parameters())[0][0].tolist()[0:3]]
        log.write(' '.join(param_r))

        generator, _, _ = sanityCheck_generator(model=generator)
        log.write('\nnew generator params:\n')
        param_g = [str(x) for x in list(generator.pretrain_model.lstm.parameters())[0][0].tolist()[0:3]]
